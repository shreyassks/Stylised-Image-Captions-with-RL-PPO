...
from app.models.backbone.swin_transformer_backbone import SwinTransformer as STBackbone
from app.models.basic_model import BasicModel
from app.models.encoder_decoder.PureT_decoder import Decoder
from app.models.encoder_decoder.PureT_encoder import Encoder
from app.utils.utils import activation, expand_tensor
from app.core.config import cfg_from_file, cfg
...
class PureT(BasicModel):
    def __init__(self):
        ...
        self.backbone.load_weights(
            'app/artifacts/swin_transformer.pth'
        )
        ...
        if cfg.MODEL.ATT_FEATS_DIM == cfg.MODEL.ATT_FEATS_EMBED_DIM:
            ...
            self.att_embed = nn.Sequential(
                ...
                activation(cfg.MODEL.ATT_FEATS_EMBED_ACT),
                nn.LayerNorm(cfg.MODEL.ATT_FEATS_EMBED_DIM) if cfg.MODEL.ATT_FEATS_NORM is True else nn.Identity(),
    ...
    def forward(self, **kwargs):
        ...
        # words mask [B, L, L]
        ##############################################
        seq_mask = (seq > 0).type(torch.IntTensor).to(device)  # cuda
        seq_mask[:, 0] += 1
        seq_mask = seq_mask.unsqueeze(-2)
        seq_mask = seq_mask & subsequent_mask(seq.size(-1)).to(seq_mask)
        seq_mask = seq_mask.type(torch.FloatTensor).to(device)  # cuda
        ##############################################
    ...
    def get_logprobs_state(self, **kwargs):
        ...
        seq_mask = subsequent_mask(ys.size(1)).to(encoder_out.device).type(torch.FloatTensor).to(device)[:, -1,
                   :].unsqueeze(
            1)  # cuda
    ...
    def decode_beam(self, **kwargs):
        ...
        seq_logprob = torch.zeros((batch_size, 1, 1)).to(cfg.INFERENCE.DEVICE)
        log_probs = []
        selected_words = None
        seq_mask = torch.ones((batch_size, beam_size, 1)).to(cfg.INFERENCE.DEVICE)
        ...
        state = None
        wt = Variable(torch.zeros(batch_size, dtype=torch.long).to(cfg.INFERENCE.DEVICE))
    ...
    def decode(self, **kwargs):
        ...
        state = None
        sents = Variable(torch.zeros((batch_size, cfg.MODEL.SEQ_LEN), dtype=torch.long).to(device))
        logprobs = Variable(torch.zeros(batch_size, cfg.MODEL.SEQ_LEN).to(device))
        wt = Variable(torch.zeros(batch_size, dtype=torch.long).to(device))
        unfinished = wt.eq(wt)
        kwargs[cfg.PARAM.ATT_FEATS] = encoder_out
        kwargs[cfg.PARAM.GLOBAL_FEAT] = gx
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if greedy_decode:
                ...
                ps.append(probs_t)
                # print(f"Probs: {probs_t}, Shape: {probs_t.shape}")
