...
def pack_wrapper(module, att_feats, att_masks):
    if att_masks is not None:
        att_masks = att_masks.to("cpu")
        packed, inv_ix = sort_pack_padded_sequence(att_feats, att_masks.data.sum(1))
...
class densepembedAttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.num_personalities = 217
        self.perss_dropout = opt.drop_prob_lm  # same drop out of personality embedding as text
        self.ss_prob = 0.0  # Schedule sampling probability
        # Word Gate
        self.wordgate = nn.Sequential(nn.Linear(self.fc_feat_size, self.vocab_size+1),
                                      nn.Sigmoid())
    ...
    def clip_att(self, att_feats, att_masks):
        ...
        if att_masks is not None:
            max_len = att_masks.data.sum(1).max()
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):  
        ...
        return fc_feats, att_feats, p_att_feats, att_masks #fc_feats.unsqueeze(0), att_feats.unsqueeze(0), p_att_feats.unsqueeze(0), att_masks
    ...
    def _forward(self, rank, ofc_feats, oatt_feats, densecap, seq, att_masks=None, personality=None):
        batch_size = self.batch_size
        seq_per_img = seq.shape[0] // batch_size

        word_gate = self.wordgate(ofc_feats.squeeze())
        outputs = torch.zeros(batch_size*seq_per_img, seq.size(1) - 1, self.vocab_size+1, dtype=torch.float).to(rank) # 64x15x10452
        ...
        att_feats = torch.stack(encoder_cells).to(rank)
        ...
        if seq_per_img > 1:
            fc_feats, att_feats, p_att_feats, att_masks = utils.repeat_tensors(seq_per_img, [fc_feats, att_feats, p_att_feats, att_masks])
            rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks = utils.repeat_tensors(
                seq_per_img, [rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks])

        for i in range(seq.size(1) - 1):
            # Scheduled Sampling to avoid exposure bias
            ...
            output,  decodestate = self.get_logprobs_state(it, personality, fc_feats, att_feats, p_att_feats, 
                                                           att_masks,rp_fc_feats, rp_att_feats, rpp_att_feats,
                                                           rp_att_masks, word_gate, decodestate)
            outputs[:, i] = output
        return outputs, word_gate

    def get_logprobs_state(self, it, personality, fc_feats, att_feats, p_att_feats, att_masks,
                           rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks, wordgate, state):
        ...
        output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, 
                                  rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks)

        logitoutput = self.logit(output)
        wordgate_feats = logitoutput*wordgate
        finallogprobs = F.log_softmax(wordgate_feats, dim=1)
    ...
    def _sample_beam(self, ofc_feats, oatt_feats, densecap, att_masks=None, personality=None, opt={}):
        ...
        att_masks = torch.ones(batch_size, 80 // beam_size)
        word_gate_feats = self.wordgate(ofc_feats.squeeze())
        rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks = self._prepare_feature(ofc_feats, oatt_feats, att_masks)
        ...
        word_gate_feats = word_gate_feats.contiguous().view(batch_size, -1)
        ...
        for k in range(batch_size):
            ...
            tmp_word_gate_feats = word_gate_feats[k:k+1].expand(beam_size, word_gate_feats.size(1))
            ...
            tmp_personality = personality[k:k+1].expand(*((beam_size,)+personality.size()[1:])).contiguous() if personality is not None else None

            for t in range(1):
                if t == 0: # input <bos>
                    it = fc_feats.new_zeros([beam_size], dtype=torch.long)
                logprobs, state = self.get_logprobs_state(it, tmp_personality,tmp_fc_feats, tmp_att_feats, tmp_p_att_feats,
                                                          tmp_att_masks,tmp_rp_fc_feats, tmp_rp_att_feats, tmp_rpp_att_feats,
                                                          tmp_rp_att_masks, tmp_word_gate_feats, state)

            self.done_beams[k] = self.beam_search(state, logprobs, tmp_personality, tmp_fc_feats, tmp_att_feats, 
                                                  tmp_p_att_feats, tmp_att_masks,tmp_rp_fc_feats, tmp_rp_att_feats, 
                                                  tmp_rpp_att_feats, tmp_rp_att_masks, tmp_word_gate_feats, opt=opt)
    ...
    def _sample(self, rank, ofc_feats, oatt_feats, densecap, att_masks=None, personality=None, opt={}):

        sample_method = opt.get('sample_method', 'greedy')
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        opt['block_trigrams'] = 1
        ...
        alogprobs1 = torch.zeros(batch_size,self.seq_length+1, self.vocab_size+1).to(rank)
        alogprobs= torch.zeros(batch_size,self.seq_length+1, self.vocab_size+1).to(rank)
        
        for bk in range(alogprobs.size(0)):
            alogprobs[bk]=nn.LogSoftmax(dim=1)(alogprobs1[bk])
            
        seq = torch.zeros((batch_size, self.seq_length), dtype=torch.long).to(rank)
        seqLogprobs = torch.zeros(batch_size, self.seq_length,dtype=torch.float).to(rank)
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                it = fc_feats.new_zeros(batch_size, dtype=torch.long)
            logprobs, decodestate = self.get_logprobs_state(it, personality, p_fc_feats, p_att_feats, pp_att_feats, 
                                                            p_att_masks,rp_fc_feats,rp_att_feats, rpp_att_feats, 
                                                            rp_att_masks, word_gate, decodestate)
            if no_unk==1:
                mask2 = torch.zeros(logprobs.size(), requires_grad=False).to(rank)
    ...
    def _samplen(self, ofc_feats, oatt_feats, densecap, att_masks=None, personality=None, opt={}):
        ...
        sample_method = opt.get('sample_method', 'sample')
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.5)
        opt['block_trigrams'] = 1
        opt['remove_bad_endings'] = 1
        opt['decoding_constraint'] = 1
        decoding_constraint = opt.get('decoding_constraint', 1)
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, decodestate = self.get_logprobs_state(it,personality, p_fc_feats, p_att_feats, pp_att_feats,
                                                            p_att_masks,rp_fc_feats,rp_att_feats, rpp_att_feats, 
                                                            rp_att_masks, decodestate, word_gate)
            if decoding_constraint and t > 0:
                tmp = logprobs.new_zeros(logprobs.size())
                tmp.scatter_(1, seq[:,t-1].data.unsqueeze(1), float('-10e20'))
                logprobs = logprobs + tmp
    ...
    def get_seq_logprobs(self, rank, ofc_feats, oatt_feats, densecap, seq, att_masks=None, personality=None):
        seq_logprobs = []
        ...
        logprobs,  decodestate = self.get_logprobs_state(it, personality, fc_feats, att_feats, p_att_feats, 
                                                             att_masks,rp_fc_feats, rp_att_feats, rpp_att_feats,
                                                             rp_att_masks, word_gate, decodestate)
        ...
        for i in range(seq.size(1) - 1):
            ...
            sample_logprobs = logprobs.gather(1, Variable(seq[:, i+1:i+2])) * Variable((seq[:, i:i+1] > 0).float(), 
                                                                                       requires_grad=False)
            seq_logprobs.append(sample_logprobs.view(-1))
...
class TopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks, rp_fc_feats, rp_att_feats, rpp_att_feats, rp_att_masks):
        ...
        # for resnext feature
        rp_fc_feats = rp_fc_feats.squeeze(0)
        rp_att_feats = rp_att_feats.squeeze(0)
        rpp_att_feats = rpp_att_feats.squeeze(0)
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        ...
        att_masks = torch.ones(att_feats.size(0), att_size)
        ...
        if att_masks is not None:
            weight = weight * att_masks.view(-1, att_size).cuda().float()
