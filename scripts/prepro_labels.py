...
def main(params):
    ...
    for i,img in enumerate(imgs):
        ...
        if 'filename' in img: jimg['file_path'] = os.path.join(img.get('filepath', '')) # copy it over, might need
        if 'cocoid' in img:
        ...
        if params['images_root'] != '':
            with Image.open(os.path.join(params['images_root'], img['filepath'])) as _img:
...
if __name__ == "__main__":
  ...
  # input json
  parser.add_argument('--input_json',default='data/dataset_person.json ', required=True, help='input json file to process into hdf5')
